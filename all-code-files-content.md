...
{% block content %}
    <div class="timeline-container">
        ...
        <div id="timeline-root" class="timeline-content h-screen"></div>
    </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/lucide/0.263.1/umd/lucide-react.js"></script>

<!-- Gestionnaire de modales -->
<script src="{{ url_for('static', filename='js/project/timeline.js') }}"></script>

<!-- Composant Timeline -->
{% include 'project/timeline-component.html' %}
{% endblock %}
```
    ...
    <div className="relative h-20 border-b">
      {project.tasks.map((task, taskIndex) => {
        ...
        return (
          <div
            key={task.id}
            className={`absolute h-12 rounded-lg p-2 text-white text-sm bg-${task.color} 
                       cursor-pointer transition-all hover:brightness-110 hover:-translate-y-0.5`}
            style={{
              ...position,
              top: taskIndex * 16 + 'px'
            }}
          ...
          >
            {task.text}
            <div className="task-tooltip opacity-0 group-hover:opacity-100">
              <span className="dates">{task.dates}</span>
              {task.comment && <span className="comment">{task.comment}</span>}
            </div>
          </div>
